...
class TestSeamus(TestCase):
    ...
    def test_use_function(self):
        ...
        self.assertEquals(self._seamus._original_func, self._original_func)
        self.assertEquals(self._seamus._original_func_args, (1, 2))
        self.assertEquals({'kwarg1': 'test', 'kwarg2': 12}, self._seamus._original_func_kwargs)

    def test_test_function(self):
        self._seamus.test(self._refactored_func, *self._get_args(), **self._get_kwargs())
        self.assertEquals(self._seamus._refactored_func, self._refactored_func)
        self.assertEquals(self._seamus._refactored_func_args, (1, 2))
        self.assertEquals({'kwarg1': 'test', 'kwarg2': 12}, self._seamus._refactored_func_kwargs)
    ...
    def test_decorator(self):
        self.assertEquals(self._decorated_function(*self._get_args(), **self._get_kwargs()), 1)
